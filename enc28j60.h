...
// 2023-12-11 Nils:
// adding SPI.beginTransaction and SPI.endTransaction
...
public:
  ...
  static bool tagging_enabled;  // Flag for enabling VLAN tagging
  static bool received_tagged;  // was the last received packet tagged?
  static uint16_t vlanID;       // VLAN ID for check and sending tagged packets
  static uint8_t vlan_TCI_PCP;  // 3 bit value: Priority Code Point 
  static uint8_t vlan_TCI_DEI;  // 1 bit value: Drop Eligible Indicator
  ...
  /**   @brief  Initialise SPI interface
    *     @note   Configures Arduino pins as input / output, etc.
    */
  static void initSPI(SPIClass* mySPI);
  ...
  /**   @brief  Wake ENC28J60 from sleep mode
    */
  static void powerUp(bool waitForLink = true);  // contrib by Alex M.

  /**   @brief  Check sleep mode / power styte
    */
  static bool isPoweredUp();
  ...
  /** @brief enables VLAN tagging
        @param ID VLAN ID
     */
  static void enable_VLAN_tagging(uint16_t ID);

  /** @brief disables VLAN tagging
     */
  static void disable_VLAN_tagging();

  /** @brief check if VLAN tagging is enabled
     *  @return <i>bool</i> true if VLAN tagging is enabled, false else.
     */
  static bool is_VLAN_tagging_enabled();

  /** @brief Was the last received packet tagged with the current set VLAN ID?
     *  @return <i>bool</i> true if the last packet was tagged with the used VLAN ID, false else.
     */
  static bool packet_Received_Was_Tagged();
};
