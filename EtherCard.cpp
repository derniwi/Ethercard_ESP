...
// 2023-12-08 Update by Nils:
// added the possibility to use DHCP in a non-blocking mode
...
//uint8_t EtherCard::timeip[IP_LEN];        // ip address of time or ntp server
...
byte EtherCard::dhcpState = 0;     // start with init state
...
bool EtherCard::staticSetup(const uint8_t* my_ip,
  ...
  if (my_ip != 0) copyIp(myip, my_ip);
  if (gw_ip != 0) setGwIp(gw_ip);
  if (dns_ip != 0) copyIp(dnsip, dns_ip);
  if (mask != 0) copyIp(netmask, mask);
